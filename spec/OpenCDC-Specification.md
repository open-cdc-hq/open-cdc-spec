...
- [4. Schema Delivery -- OBJECT_METADATA and Producer Schema Modes](#4-schema-delivery----object_metadata-and-producer-schema-modes)
  - [4.1 Mandatory Baseline: Schema-Before-First-Use](#41-mandatory-baseline-schema-before-first-use)
...
- [6. Producer Contract](#6-producer-contract)
  ...
  - [6.4 Consumer Reconnection Obligations](#64-consumer-reconnection-obligations)
...
- `"committed_only"` (default when absent): all emitted events belong to transactions that have been committed at the source. This version of the specification defines only this value. When the field is absent, the stream is treated as `"committed_only"`. See Appendix A.10 for guidance on handling unrecognized values.
...
- P-TRX-6: A producer MUST declare transaction_visibility in STREAM_METADATA. In this version only "committed_only" is a conformant value. A producer that omits the field produces a stream that is interpreted as "committed_only" (see Appendix A.10 for consumer-side guidance on unrecognized values).
...
Transaction boundary markers (TRX_COMMIT) are permitted in Ephemeral Mode under the same rules as Durable Mode. Producers MAY declare transaction_boundaries and emit markers; when declared, the producer obligations in Section 10.5 apply. The consumer completeness guarantee in condition (c) is weakened by Ephemeral Mode's at-most-once delivery: a missing marker is consistent with the declared data-loss tolerance and is not an error condition. See Appendix A.10 (C-TRX-5) for consumer-side guidance.
...
- **R-POS-0**
  - Requirement: Ordering is guaranteed within a partitionkey; total cross-table ordering requires the CloudEvents sequence field
  - Who: Consumer
  - Section: 8.2
